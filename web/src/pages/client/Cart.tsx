    ...
    return (
      <div className="min-h-screen bg-background flex flex-col">
        <header className="fixed top-0 left-0 right-0 z-50 px-4 py-3 flex items-center bg-background/80 backdrop-blur-xl border-b border-border">
          ...
          <h1 className="text-lg font-bold font-display">{t("pages:cart.title")}</h1>
        </header>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 pt-20 px-4 text-center">
          ...
          <h2 className="text-xl font-bold font-display">{t("pages:cart.emptyTitle")}</h2>
          <p className="text-muted-foreground text-sm">{t("pages:cart.emptyHint")}</p>
          <Button asChild className="mt-2">
            <Link to="/shop">{t("pages:cart.browseProducts")}</Link>
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="fixed top-0 left-0 right-0 z-50 px-4 py-3 flex items-center bg-background/80 backdrop-blur-xl border-b border-border">
        ...
        <h1 className="text-lg font-bold font-display">{t("pages:cart.titleWithCount", { count: items.length })}</h1>
      </header>
      ...
      <div className="client-page-container client-page-content pt-20 pb-56 lg:pb-36 space-y-3">
        {items.map(({ product, quantity }) => {
          ...
          return (
            <div key={product.id} className="floating-card p-3 flex gap-3 items-start">
              {image ? (
                <img src={image} alt={product.name} className="w-20 h-20 rounded-xl object-cover flex-shrink-0" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-primary font-bold mt-1">
                  {t("common:currencyShort")} {Number(product.selling_price) * quantity}
                </p>
                <p className="text-muted-foreground text-xs">{t("pages:cart.each", { price: product.selling_price })}</p>
                {stock > 0 ? (
                  <p className="text-muted-foreground text-[11px] mt-0.5">
                    {t("pages:cart.maxStock", { stock })}
                    {cannotIncrease ? t("pages:cart.maxStockCart") : ""}
                  </p>
                ) : null}
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-2 bg-muted rounded-xl p-1">
                    ...
                    <button
                      ...
                      className="w-7 h-7 rounded-lg bg-background flex items-center justify-center disabled:opacity-40"
                      aria-label={cannotIncrease ? t("pages:cart.ariaMaxQty") : t("pages:cart.ariaIncreaseQty")}
                    >
      ...
      <div className="fixed bottom-16 left-0 right-0 p-4 lg:px-8 bg-background border-t border-border space-y-3 z-40">
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">{t("pages:cart.subtotal")}</span>
        ...
        <div className="flex gap-3 lg:max-w-2xl lg:ml-auto">
          <Button variant="outline" asChild className="flex-1">
            <Link to="/shop">{t("pages:cart.continueShopping")}</Link>
          </Button>
