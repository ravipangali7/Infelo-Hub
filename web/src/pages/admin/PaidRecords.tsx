...
import { useAdminPaidRecordList, adminKeys } from "@/api/hooks";
import { useQueryClient } from "@tanstack/react-query";
import { adminApi } from "@/api/endpoints";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import type { PaidRecord } from "@/api/types";

const PAGE_SIZE = 20;

export default function PaidRecords() {
  ...
  const { data, isLoading, error } = useAdminPaidRecordList(params);
  ...
  if (isLoading && results.length === 0) {
    return (
      <div className="space-y-4">
        <div><h1 className="text-2xl font-bold">Paid Records</h1><p className="text-muted-foreground">Manage paid records</p></div>
  ...
  const columns = [
    {
      ...
      render: (row: { id: number }) => <Link className="text-primary underline font-medium" to={`/system/paid-records/${row.id}`}>#{row.id}</Link>,
    },
    { id: "name", label: "Name", sortKey: "name" },
    {
      id: "user_col",
      label: "User",
      render: (row: PaidRecord) => (
    ...
    { id: "created_at", label: "Created", sortKey: "created_at", render: (row: { created_at: string }) => new Date(row.created_at).toLocaleDateString() },
    {
      ...
      render: (row: { id: number }) => (
        <AdminRowActions
          viewHref={`/system/paid-records/${row.id}`}
          editHref={`/system/paid-records/${row.id}/edit`}
          onDelete={async () => {
            await adminApi.deletePaidRecord(row.id);
            qc.invalidateQueries({ queryKey: adminKeys.paidRecords(params) });
          }}
          deleteConfirmLabel="Delete this paid record? This action cannot be undone."
        />
      ),
    },
  ];
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h1 className="text-xl md:text-2xl font-bold">Paid Records</h1>
          <p className="text-muted-foreground text-xs md:text-sm">Manage paid records</p>
        </div>
        <Button asChild className="w-full sm:w-auto shrink-0">
          <Link to="/system/paid-records/new">Add</Link>
        </Button>
      </div>
      ...
      <Card>
        <CardContent className="p-0">
          <AdminDataTable columns={columns} data={results} keyFn={(row) => (row as { id: number }).id} orderBy={orderBy} onOrderByChange={setOrderBy} isLoading={isLoading} emptyMessage="No paid records." />
        </CardContent>
